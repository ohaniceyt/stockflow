-- Harden complete_sale: validate unit prices against the trusted product
-- selling_price at the database level. This is the second line of defense
-- after the Edge Function validation.
...
BEGIN
  ...
  LOOP
    ...
    -- Ensure the product belongs to the organization and fetch its trusted price.
    SELECT selling_price INTO v_trusted_unit_price
    FROM products
    WHERE id = v_product_id AND org_id = p_org_id AND is_active = TRUE;

    IF v_trusted_unit_price IS NULL THEN
      RAISE EXCEPTION 'Produit invalide ou inactif';
    END IF;

    IF ABS(v_unit_price - v_trusted_unit_price) > v_epsilon THEN
      RAISE EXCEPTION 'Prix unitaire non conforme pour le produit % (attendu %, reçu %)', v_product_id, v_trusted_unit_price, v_unit_price;
    END IF;
