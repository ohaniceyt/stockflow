-- Fix function overloading caused by migration 00000000000049_cashier_atomic_checkout.sql.
-- That migration added a 12-parameter record_movement(p_reference_id), while
-- 20260724000000 recreated an 11-parameter version with org-scoped idempotency.
-- PostgREST cannot choose between the two signatures when the Edge Function calls
-- record_movement without p_reference_id, producing:
--   PGRST203: Could not choose the best candidate function.
--
-- Dropping the 12-arg signature (as the previous version of this migration did) would
-- break cashier_atomic_checkout (migration 00000000000049, already applied in prod),
-- which calls record_movement positionally with 12 args (the 12th = v_receipt_id, bound
-- to p_reference_id). Every cashier sale would then 500.
--
-- Solution: collapse both signatures into ONE canonical 12-arg record_movement that
-- combines:
--   * the org-scoped idempotency from 20260724000000 (cross-org client_operation_id
--     reuse no longer returns another org's movement), and
--   * the p_reference_id parameter from 00000000000049 (cashier tags each OUT movement
--     with the receipt id).
-- The Edge Function's 11-named-arg rpc call still matches (p_reference_id DEFAULT NULL),
-- and cashier_atomic_checkout's 12-positional-arg call still matches. No caller change.

DROP FUNCTION IF EXISTS public.record_movement(
  UUID, UUID, UUID, UUID, TEXT, INTEGER, TEXT, UUID, NUMERIC, UUID, UUID
); -- 11-arg (20260724000000)
DROP FUNCTION IF EXISTS public.record_movement(
  UUID, UUID, UUID, UUID, TEXT, INTEGER, TEXT, UUID, NUMERIC, UUID, UUID, UUID
); -- 12-arg (00000000000049)
...
BEGIN
  ...
  IF p_quantity < 0 OR (p_quantity = 0 AND p_type NOT IN ('ADJUSTMENT', 'INVENTORY')) THEN
  ...
  -- operation id. Scope the lookup by org so cross-org reuse does not return a
  -- movement belonging to another organization.
...
-- Hardened search_path (public, pg_catalog) — aligned with the 20260705000000
-- search_path-hardening pass; excludes pg_temp to prevent search_path injection.
ALTER FUNCTION public.record_movement(UUID, UUID, UUID, UUID, TEXT, INTEGER, TEXT, UUID, NUMERIC, UUID, UUID, UUID) SET search_path = public, pg_catalog;
