-- Fix TRANSFER idempotency: a single client operation id legitimately produces two
-- movement rows (source and target). Replace the partial unique index with a
-- composite key so both rows are allowed while still preventing duplicate
-- non-null operation ids for the same product/location pair.
-- Also allow ADJUSTMENT/INVENTORY to set stock to zero.

DROP INDEX IF EXISTS movements_client_operation_id_idx;

CREATE UNIQUE INDEX IF NOT EXISTS movements_client_operation_id_location_idx
  ON movements (org_id, client_operation_id, location_id, target_location_id)
  WHERE client_operation_id IS NOT NULL;

-- Re-create record_movement with the relaxed quantity check.
